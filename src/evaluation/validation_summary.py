...
def _validation_status_from_gates(row: pd.Series, gate_cols: Iterable[str]) -> str:
    gates = [row.get(c) for c in gate_cols]
...
def _validation_status(row: pd.Series) -> str:
    """Primary twin-validation status (G1..G5).

    By design this excludes G6, which is retained as an auxiliary statistical
    screen (`stat_screen_pass`) rather than a main engineering veto — matching
    the canonical FULLSQUARE methodology (docs/reference/PROTOCOL.md).
    """
...
def _gate_pass_ratio(
    df_group: pd.DataFrame, gate_cols: Iterable[str] | None = None
) -> float:
