...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=REPO_ROOT,
        help="Repository root. Defaults to the canonical repo containing this script.",
    )
    parser.add_argument(
        "--dataset_root",
        type=Path,
...
def main() -> None:
    ...
    dataset_root = (
        ...
        if args.dataset_root is not None
            ...
            (repo / 'data' / 'benchmarks' / 'modulations' / '16qam')
            if (repo / 'data' / 'benchmarks' / 'modulations' / '16qam').exists()
            else (repo / 'data' / '16qam')
        ).resolve()
    )
