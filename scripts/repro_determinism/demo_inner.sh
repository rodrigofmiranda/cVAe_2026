...
echo "[demo] DET=${CVAE_DETERMINISTIC} ENV=${CVAE_DET_ENV} OPDET=${CVAE_DET_OPDET} SETSEED=${CVAE_DET_SETSEED} sub=${REPRO_OUT_SUBDIR} ep=${REPRO_MAX_EPOCHS}"
python3 -u -m src.protocol.run \
  ...
  --keras_verbose 2 --max_epochs "${REPRO_MAX_EPOCHS}" --skip_eval \
  2>&1 | grep -vE "NUMA node|ptx85|cuda_executor|cuFFT|cuDNN factory|cuBLAS factory" || true
echo "[demo] DONE"
