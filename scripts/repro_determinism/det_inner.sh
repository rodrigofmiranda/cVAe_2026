...
echo "[det] CVAE_DETERMINISTIC=${CVAE_DETERMINISTIC} seed=${REPRO_SEED} max_epochs=${REPRO_MAX_EPOCHS} sub=${REPRO_OUT_SUBDIR}"
python3 -u -m src.protocol.run \
  ...
  --no_data_reduction \
  --seed "${REPRO_SEED}" \
  --stat_tests --stat_mode quick --stat_seed "${REPRO_SEED}" \
  --keras_verbose 2 \
  --max_epochs "${REPRO_MAX_EPOCHS}" --skip_eval
echo "[det] DONE rc=$?"
