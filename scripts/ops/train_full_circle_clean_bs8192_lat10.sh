...
usage() {
  cat <<'USAGE'
Usage:
  scripts/ops/train_full_circle_clean_bs8192_lat10.sh [extra protocol args...]

Purpose:
  Prepare the clean Full Circle restart before any border-focused adaptations.
...
Defaults:
  ...
  - preset: support_full_circle_clean_bs8192_lat10_v1
...
Environment overrides:
  ...
  OUTPUT_BASE          Output base (default: outputs/full_circle/<RUN_STAMP>_clean_bs8192_lat10_100k)
...
export OUTPUT_BASE="${OUTPUT_BASE:-$REPO_ROOT/outputs/full_circle/${RUN_STAMP}_clean_bs8192_lat10_100k}"
...
CMD=(
  ...
  --grid_preset support_full_circle_clean_bs8192_lat10_v1
  --no_data_reduction
...
printf 'Running clean Full Circle bs8192/lat10 restart\n'
printf '  dataset_root=%s\n' "$DATASET_ROOT"
printf '  output_base=%s\n' "$OUTPUT_BASE"
printf '  grid_preset=%s\n' "support_full_circle_clean_bs8192_lat10_v1"
printf '  stat_mode=%s\n' "$STAT_MODE"
printf '  patience=%s\n' "$PATIENCE"
