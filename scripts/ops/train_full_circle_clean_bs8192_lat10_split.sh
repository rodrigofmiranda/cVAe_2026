...
CONTAINER_WORKDIR="${CVAE_TF25_WORKDIR:-$REPO_ROOT}"
CONTAINER_DATASET_ROOT="${CONTAINER_DATASET_ROOT:-${CONTAINER_WORKDIR}/data/FULL_CIRCLE_2026}"
...
OUTPUT_BASE_A="${CONTAINER_WORKDIR}/outputs/full_circle/${RUN_STAMP}_clean_bs8192_lat10_100k_split_a"
OUTPUT_BASE_B="${CONTAINER_WORKDIR}/outputs/full_circle/${RUN_STAMP}_clean_bs8192_lat10_100k_split_b"
...
CVAE_TF25_WORKDIR="$CONTAINER_WORKDIR" \
...
tmux send-keys -t "$SESSION_A" \
  "cd \"${CONTAINER_WORKDIR}\" && mkdir -p \"${OUTPUT_BASE_A}/logs\" && DATASET_ROOT=\"${CONTAINER_DATASET_ROOT}\" OUTPUT_BASE=\"${OUTPUT_BASE_A}\" STAT_MODE=\"${STAT_MODE}\" PATIENCE=\"${PATIENCE}\" REDUCE_LR_PATIENCE=\"${REDUCE_LR_PATIENCE}\" scripts/ops/train_full_circle_clean_bs8192_lat10.sh --grid_tag \"^S27cov_fc_clean_lc0p25_t0p03$|^S27cov_fc_clean_lc0p25_t0p03_bs8192$\" --seed ${SEED}${EXTRA_CMD} | tee \"${LOG_A}\"" C-m

tmux send-keys -t "$SESSION_B" \
  "cd \"${CONTAINER_WORKDIR}\" && mkdir -p \"${OUTPUT_BASE_B}/logs\" && DATASET_ROOT=\"${CONTAINER_DATASET_ROOT}\" OUTPUT_BASE=\"${OUTPUT_BASE_B}\" STAT_MODE=\"${STAT_MODE}\" PATIENCE=\"${PATIENCE}\" REDUCE_LR_PATIENCE=\"${REDUCE_LR_PATIENCE}\" scripts/ops/train_full_circle_clean_bs8192_lat10.sh --grid_tag \"^S27cov_fc_clean_lc0p25_t0p03_lat10$\" --seed ${SEED}${EXTRA_CMD} | tee \"${LOG_B}\"" C-m
