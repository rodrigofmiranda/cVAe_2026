...
| `seq_bigru_residual + MDN` dedicated rerun branch | `exp_20260328_041729`, `exp_20260328_181213` | `4/12`, `2/12` | Current reproducibility is worse than the historical MDN anchors, and the exact historical candidate still collapses under the current runtime lane |
...
- The immediate blocker is now reproducibility drift:
  ...
  - data quantity matched the benchmark caps, so sample count is not the main
    explanation
...
2. Reproduce the benchmark family inside the documented `tf25_gpu` container
   path before any new MDN sweep.
