...
Foram escolhidos três representantes, um da linha `full_square` e dois da linha
`full_circle`, para permitir uma leitura comparativa direta:
...
Nesta comparação, o root canônico a ser citado é sempre o sufixo
`eval_16qam_crossline_20260420_clean`. A primeira tentativa operacional foi
descartada após falhas de infraestrutura e não deve ser usada como referência.
...
A rodada comparativa `full_square` versus `full_circle` foi concluída com
`12/12 regimes` completos para os três candidatos selecionados.
