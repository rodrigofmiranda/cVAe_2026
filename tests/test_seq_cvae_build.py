...
class TestSubModelBuild:
    ...
    def test_graybox_prior_adds_imdd_feature_layer(self):
        prior = build_seq_prior_net(_min_cfg(arch_variant="seq_imdd_graybox"))
        assert prior.get_layer("prior_net_imdd_poly") is not None

    def test_graybox_decoder_builds_physical_branch(self):
        dec = build_seq_decoder(_min_cfg(arch_variant="seq_imdd_graybox"))
...
class TestPointWiseIsolation:
    ...
    def test_seq_imdd_graybox_dispatches_via_build_cvae(self):
        from src.models.cvae import build_cvae
        cfg = {
            "layer_sizes": [32, 64], "latent_dim": 4, "beta": 0.001,
            "lr": 3e-4, "dropout": 0.0, "free_bits": 0.0,
            ...
            "window_size": 7, "seq_hidden_size": 16, "seq_num_layers": 1,
            "seq_bidirectional": True, "decoder_distribution": "gaussian",
        }
        vae, kl_cb = build_cvae(cfg)
        assert vae is not None
